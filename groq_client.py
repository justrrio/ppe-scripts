...
if __name__ == "__main__":
    ...
    if test_dir:
        test_images = [os.path.join(test_dir, f) for f in sorted(os.listdir(test_dir)) if f.endswith(".jpg")][:2]
        
        if test_images:
            print(f"Testing with {len(test_images)} images from {os.path.basename(test_dir)}:\n")
            
            # Test batch analysis
            results = client.analyze_images_batch(test_images)
            
            for r in results:
                print(f"  {os.path.basename(r['image_path'])}:")
                print(f"    has_human: {r.get('has_human')}")
        ...
        print("Run frame extraction first to generate test images")
